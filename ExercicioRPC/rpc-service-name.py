...
def serviceRegister(serviceName, host, port):
    try:
...
def unregisterService(services):
    try:
...
server.register_function(serviceRegister, "serviceRegister")
